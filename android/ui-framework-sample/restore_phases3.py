import subprocess
import re

file_path = '/Volumes/SSD/Projects/GitHub/project-ui-framework/android/ui-framework-sample/app/src/main/java/mcnc/app/ui_framework_sample/MainActivity.kt'

result = subprocess.run(
    ['git', 'show', 'HEAD:android/ui-framework-sample/app/src/main/java/mcnc/app/ui_framework_sample/MainActivity.kt'],
    capture_output=True, text=True, cwd='/Volumes/SSD/Projects/GitHub/project-ui-framework/android/ui-framework-sample'
)
original_content = result.stdout

def extract_view_factory(content, name):
    start_str = f'name = "{name}",'
    if start_str not in content:
        return None
    start_idx = content.find(start_str)
    vf_start = content.find('viewFactory = {', start_idx)
    open_braces = 0
    vf_end = -1
    for i in range(vf_start, len(content)):
        if content[i] == '{': open_braces += 1
        elif content[i] == '}':
            open_braces -= 1
            if open_braces == 0:
                vf_end = i
                break
    return content[vf_start:vf_end+1]

vf_p5s1 = extract_view_factory(original_content, "Phase 5 Sheets (Batch 1)")
vf_p6c1 = extract_view_factory(original_content, "Phase 6 Cards & Lists (Batch 1)")
vf_p6l2 = extract_view_factory(original_content, "Phase 6 Lists & Profiles (Batch 2)")

# Phase 5 Sheets (Batch 1) Fixes
# ONLY fix selectedLanguage -> selectedLang in BizLanguageSheet
vf_p5s1 = vf_p5s1.replace('selectedLanguage = selectedLanguage', 'selectedLang = selectedLanguage')

# Phase 6 Cards & Lists (Batch 1) Fixes
# BizAccountSavingCard: isSelected -> selected, onClick -> onSelect
vf_p6c1 = re.sub(r'BizAccountSavingCard\((.*?)\)', lambda m: 'BizAccountSavingCard(' + m.group(1).replace('isSelected =', 'selected =').replace('onClick =', 'onSelect =') + ')', vf_p6c1, flags=re.DOTALL)
# No other changes needed for Phase 6 Batch 1! BizAccountListCard is perfectly fine. BizActionCard is perfectly fine. BizUpgradeCard, BizVerificationCard are fine.

# Phase 6 Lists & Profiles (Batch 2) Fixes
# BizTransferList: modelValue -> targetKeys, onUpdateModelValue -> onTargetKeysChange, variant = "outlined" -> variant = kh.mcnc.lib.components.BizTransferVariant.Outlined
vf_p6l2 = re.sub(r'BizTransferList\((.*?)\)', lambda m: 'BizTransferList(' + m.group(1).replace('modelValue =', 'targetKeys =').replace('onUpdateModelValue =', 'onTargetKeysChange =').replace('variant = "outlined"', 'variant = kh.mcnc.lib.components.BizTransferVariant.Outlined') + ')', vf_p6l2, flags=re.DOTALL)

# BizFabList and BizFabAction fixes
vf_p6l2 = vf_p6l2.replace('BizFabList(\n                                    side = kh.mcnc.lib.components.BizFabSide.Top,\n                                    isActive = isFabActive', 'BizFabList(\n                                    side = kh.mcnc.lib.components.BizFabSide.Top,\n                                    active = isFabActive')
vf_p6l2 = vf_p6l2.replace('color = "light"', 'color = kh.mcnc.lib.components.BizFabActionColor.Light')
vf_p6l2 = vf_p6l2.replace('color = "primary"', 'color = kh.mcnc.lib.components.BizFabActionColor.Primary')
vf_p6l2 = vf_p6l2.replace('color = "danger"', 'color = kh.mcnc.lib.components.BizFabActionColor.Danger')
vf_p6l2 = re.sub(
    r'icon = \{\s*Icon\(\s*imageVector = (if \(isFabActive\) Icons\.Default\.Close else Icons\.Default\.Add),\s*contentDescription = null\s*\)\s*\}',
    r'icon = \1',
    vf_p6l2
)

# Patch current file
with open(file_path, 'r') as f:
    current_content = f.read()

def patch_block(content, name, new_vf):
    start_str = f'name = "{name}",'
    if start_str not in content:
        return content
    start_idx = content.find(start_str)
    vf_start = content.find('viewFactory = {', start_idx)
    open_braces = 0
    vf_end = -1
    for i in range(vf_start, len(content)):
        if content[i] == '{': open_braces += 1
        elif content[i] == '}':
            open_braces -= 1
            if open_braces == 0:
                vf_end = i
                break
    return content[:vf_start] + new_vf + content[vf_end+1:]

current_content = patch_block(current_content, "Phase 5 Sheets (Batch 1)", vf_p5s1)
current_content = patch_block(current_content, "Phase 6 Cards & Lists (Batch 1)", vf_p6c1)
current_content = patch_block(current_content, "Phase 6 Lists & Profiles (Batch 2)", vf_p6l2)

# Add missing Icons imports
imports_to_add = """import androidx.compose.material.icons.filled.SwapHoriz
import androidx.compose.material.icons.filled.Edit
import androidx.compose.material.icons.filled.Folder
import androidx.compose.material.icons.filled.Delete
import androidx.compose.material.icons.filled.Close
import androidx.compose.material.icons.filled.Add
"""

if 'import androidx.compose.material.icons.filled.SwapHoriz' not in current_content:
    current_content = current_content.replace('import androidx.compose.material.icons.Icons', imports_to_add + '\nimport androidx.compose.material.icons.Icons')

with open(file_path, 'w') as f:
    f.write(current_content)
