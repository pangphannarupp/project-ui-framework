<template>
  <div 
    class="pp-swipe-wrapper"
    @touchstart="onTouchStart"
    @touchmove="onTouchMove"
    @touchend="onTouchEnd"
  >
    <div class="pp-swipe-actions left" ref="leftActionsRef">
      <slot name="left"></slot>
    </div>
    
    <div class="pp-swipe-actions right" ref="rightActionsRef">
      <slot name="right"></slot>
    </div>

    <div 
      class="pp-swipe-content" 
      :style="contentStyle"
    >
      <slot></slot>
    </div>
...
.pp-swipe-actions.left {
  left: 0;
}

.pp-swipe-actions.right {
  right: 0;
}

/* Common style for buttons inside slots to stretch full height */
.pp-swipe-actions :deep(> *) {
  display: flex;
  align-items: center;
