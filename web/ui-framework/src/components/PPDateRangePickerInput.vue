    ...
    <div class="trigger" @click.stop="toggleCalendar">
      <slot name="trigger" :value="formattedText">
        <div class="default-input">
          ...
          />
          <button v-if="modelValue && (modelValue.start || modelValue.end) && clearable" @click.stop="clearValue" class="clear-btn">
...
const props = withDefaults(defineProps<{
  modelValue?: DateRangeVal | null;
  placeholder?: string;
  clearable?: boolean;
  format?: (val: DateRangeVal) => string;
}>(), {
  modelValue: null,
...
const emit = defineEmits<{
  (e: 'update:modelValue', value: DateRangeVal | null): void;
}>();
...
const formattedText = computed(() => {
  ...
  if (v.start && !v.end) return `${formatD(v.start)} to ?`;
...
const onRangeUpdate = (payload: DateRangeVal) => {
  emit('update:modelValue', payload);
  // Auto-close if both are selected (optional, but typical)
  if (payload.start && payload.end && !props.modelValue?.end) {
    isOpen.value = false;
  }
};
