<template>
  <label class="biz-radio-label">
    <div :class="['biz-radio', { 'biz-radio--checked': isChecked }]">
      <input
        type="radio"
        class="biz-radio-input"
        :checked="isChecked"
        :value="value"
        :name="name"
        @change="onChange"
      />
      <div class="biz-radio-inner"></div>
    </div>
    <span class="biz-radio-text" v-if="label || $slots.default">
      <slot>{{ label }}</slot>
    </span>
  </label>
</template>
...
.biz-radio--checked .biz-radio-inner {
  opacity: 1;
  transform: scale(1);
}
