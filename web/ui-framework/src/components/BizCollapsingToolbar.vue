<template>
  <div class="biz-collapsing-wrapper">
    <!-- Header Layer -->
    <div 
      class="biz-collapsing-header"
      :style="{
    ...
    >
      ...
      >
        ...
        <div 
          class="biz-collapsing-title"
          :style="titleStyle"
        >
          <slot name="title">{{ title }}</slot>
        </div>

        <div class="biz-toolbar-end">
          <slot name="end"></slot>
        </div>
      </div>
    </div>
    ...
    <!-- Scrollable Content -->
    <div 
      class="biz-collapsing-content"
      @scroll="onScroll"
      ref="scrollArea"
    >
      <div class="biz-collapsing-spacer" :style="{ height: `calc(${expandedHeight}px + var(--ion-safe-area-top, 0px))` }"></div>
      <div class="biz-collapsing-inner-content">
        <slot></slot>
      </div>
    </div>
  </div>
...
.biz-collapsing-bg :deep(img) {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

.biz-collapsing-toolbar {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
...
.biz-collapsing-spacer {
  width: 100%;
  flex-shrink: 0;
}

.biz-collapsing-inner-content {
  position: relative;
