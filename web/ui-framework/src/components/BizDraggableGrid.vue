<template>
  ...
  >
    ...
    <div v-if="modelValue.length === 0" class="biz-draggable-empty-state">
      <slot name="empty">
...
.biz-draggable-item:active {
  cursor: grabbing;
}

.biz-draggable-item.is-dragging {
  opacity: 0.4;
  transform: scale(0.95);
}

.biz-draggable-item.is-drag-over {
  outline: 2px dashed #0056b3;
  outline-offset: 2px;
  border-radius: 4px;
}

.biz-draggable-empty-state {
  grid-column: 1 / -1;
  display: flex;
  align-items: center;
