<template>
  <div class="biz-bar-chart" :style="{ height: height + 'px' }">
    <div class="biz-bar-chart-inner">
      <div 
        v-for="(item, index) in data" 
        :key="index"
        class="biz-bar-wrapper"
      >
        <div class="biz-bar-value" v-if="showValues">{{ item.value }}</div>
        <div 
          class="biz-bar"
          :style="{
        ...
        ></div>
        <div class="biz-bar-label" v-if="item.label">{{ item.label }}</div>
      </div>
    </div>
  </div>
</template>
...
export interface BizBarChartData {
  label?: string;
  value: number;
  color?: string;
