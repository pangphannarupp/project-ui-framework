<template>
  <div class="biz-switch-wrapper" :class="[`variant-${variant}`, { 'is-disabled': disabled }]">
    <ion-toggle
      class="biz-switch"
      :checked="modelValue"
      :disabled="disabled"
    ...
    >
      ...
      <span class="biz-switch-label" v-if="label">{{ label }}</span>
      <slot></slot>
    </ion-toggle>
  </div>
</template>
...
.biz-switch {
  ...
  --track-background-checked: var(--biz-primary, #003399);
}

/* Material 3 Variant */
.variant-m3 .biz-switch {
  --track-background: #e0e0e0;
  --track-background-checked: var(--biz-primary, #003399);
  --handle-background: var(--biz-primary-variant, #1a2a5e);
...
.variant-m3 .biz-switch::part(track) {
  height: 32px;
  width: 52px;
  border-radius: 16px;
...
.variant-m3 .biz-switch:not(.toggle-checked)::part(track) {
  border-color: #757575;
}

.variant-m3 .biz-switch:not(.toggle-checked)::part(handle) {
  background: #757575;
  transform: scale(0.66);
}

/* Icon Variant */
.variant-icon .biz-switch::part(handle) {
  display: flex;
  align-items: center;
  justify-content: center;
}
.variant-icon .biz-switch::part(handle)::after {
  content: '';
  display: block;
  width: 12px;
  height: 12px;
...
.variant-icon .biz-switch:not(.toggle-checked)::part(handle)::after {
...
.variant-icon .biz-switch.toggle-checked::part(handle)::after {
