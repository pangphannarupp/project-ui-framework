<template>
  <Teleport to="body">
    <transition :name="`island-fade-${position}`">
      <div 
        v-if="modelValue" 
        class="biz-dynamic-island-wrapper"
        :class="[`is-position-${position}`, wrapperClass]"
        :style="wrapperStyle"
      >
        <div 
          ref="islandRef"
          class="biz-dynamic-island" 
...
const islandDynamicStyle = computed(() => {
  ...
  if (props.bgColor) {
    styles['--biz-island-bg'] = props.bgColor;
  }
  if (props.textColor) {
    styles['--biz-island-color'] = props.textColor;
  }
  return styles;
});
...
.biz-dynamic-island-wrapper.is-position-top,
.biz-dynamic-island-wrapper.is-position-bottom {
  left: 50%;
  transform: translateX(-50%);
  justify-content: center;
}

.biz-dynamic-island-wrapper.is-position-left,
.biz-dynamic-island-wrapper.is-position-right {
  top: 50%;
...
.biz-dynamic-island {
  background-color: var(--biz-island-bg, #000000);
  color: var(--biz-island-color, #ffffff);
...
/* State Dimensions */
.biz-dynamic-island.is-minimal {
  width: 48px;
  height: 48px;
  border-radius: 24px;
}

.biz-dynamic-island.is-compact {
  width: 220px;
  height: 40px;
  border-radius: 20px;
  padding: 0 16px;
...
.biz-dynamic-island.is-expanded {
  width: var(--biz-island-expanded-width, 340px);
  height: var(--biz-island-expanded-height, 180px);
  border-radius: var(--biz-island-expanded-radius, 36px);
  padding: 20px;
...
.biz-dynamic-island.is-full-width:not(.is-minimal) {
...
.minimal-dot {
  ...
  height: 12px;
  border-radius: 50%;
  background-color: var(--biz-island-accent, #2dd36f);
}
