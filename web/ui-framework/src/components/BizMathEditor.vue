<template>
  <div class="biz-math-container" :class="{ 'is-focused': isFocused }">
    <!-- Toolbar -->
    <div class="biz-math-toolbar">
    ...
    <!-- Input Area -->
    <div class="biz-math-input-wrapper">
      <textarea
        ref="textareaRef"
        class="biz-math-input"
        :value="modelValue"
        @input="onInput"
    ...
    <!-- Preview Area -->
    <div v-if="!hidePreview" class="biz-math-preview">
      <div v-if="!katexLoaded" class="biz-math-loading">Loading math renderer...</div>
      <div v-else ref="previewRef" class="biz-math-rendered"></div>
    </div>
  </div>
</template>
...
// Render logic
const renderMath = () => {
  if (!katexLoaded.value || !previewRef.value || props.hidePreview) return;
  
  try {
    // @ts-ignore - katex will be loaded globally
    window.katex.render(props.modelValue || ' ', previewRef.value, {
      throwOnError: false,
      displayMode: true // Centers the equation nicely
    });
  } catch (err) {
...
// Load KaTeX dynamically to avoid huge bundle sizes
onMounted(() => {
  // @ts-ignore
  if (window.katex) {
