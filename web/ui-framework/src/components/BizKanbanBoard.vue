<template>
  <div class="biz-kanban-board">
    <div 
      v-for="(column, colIndex) in modelValue" 
      :key="column.id" 
      class="biz-kanban-column"
      @dragover.prevent="onDragOverColumn(colIndex)"
      @drop.prevent="onDropColumn(colIndex)"
    >
      <div class="biz-kanban-column-header">
        <h3 class="biz-kanban-column-title">{{ column.title }}</h3>
        <span class="biz-kanban-column-count">{{ column.cards.length }}</span>
      </div>

      <div class="biz-kanban-column-content">
        <transition-group name="kanban-list" tag="div" class="biz-kanban-list">
          <div
            v-for="(card, cardIndex) in column.cards"
            :key="card.id"
            class="biz-kanban-card-wrapper"
            draggable="true"
          ...
          >
            ...
            <slot name="card" :card="card" :column="column">
              <div class="biz-kanban-default-card">
...
.biz-kanban-card-wrapper {
  cursor: grab;
  user-select: none;
}

.biz-kanban-card-wrapper:active {
  cursor: grabbing;
}

.biz-kanban-card-wrapper.is-dragging {
  opacity: 0.4;
}

.biz-kanban-default-card {
  background: white;
  padding: 12px 16px;
  border-radius: 6px;
