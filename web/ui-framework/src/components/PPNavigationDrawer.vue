<template>
  <Teleport to="body">
    <transition name="drawer-fade">
      <div v-if="modelValue && type === 'modal'" class="pp-drawer-backdrop" @click="closeDrawer"></div>
    </transition>
    
    <transition name="drawer-slide">
      <div v-if="modelValue || type === 'standard'" class="pp-navigation-drawer" :class="[type, { 'is-open': modelValue }]">
...
.pp-navigation-drawer {
  ...
  width: 360px;
  max-width: 85vw;
  background-color: var(--pp-drawer-bg, #f3f4f9); /* M3 Surface container low */
  z-index: 101;
  display: flex;
...
.pp-navigation-drawer.standard.is-open {
  display: flex;
}
...
.drawer-title {
  ...
  font-size: 14px;
  font-weight: 500;
  color: var(--pp-drawer-title-color, #44474f); /* M3 On-surface variant */
...
.drawer-item {
  ...
  color: var(--pp-drawer-item-color, #1a1b21); /* M3 On-surface */
}
...
.drawer-item.is-active {
  background-color: var(--pp-drawer-active-bg, #d8e2ff); /* M3 Secondary container */
  color: var(--pp-drawer-active-color, #001a41); /* M3 On-secondary container */
}
...
.drawer-item-icon {
  ...
  height: 24px;
  margin-right: 12px;
  color: var(--pp-drawer-icon-color, #44474f); /* M3 On-surface variant */
}
...
.drawer-item-badge {
  background-color: var(--pp-drawer-badge-bg, #b3261e);
  color: var(--pp-drawer-badge-color, #ffffff);
  font-size: 11px;
  font-weight: 500;
  padding: 0 6px;
  border-radius: 8px;
