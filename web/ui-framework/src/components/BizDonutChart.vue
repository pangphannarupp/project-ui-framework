<template>
  <div class="biz-donut-chart" :style="{ width: size + 'px', height: size + 'px' }">
    ...
    <!-- Center Content -->
    <div class="biz-donut-center" v-if="$slots.default || label || value">
      <slot>
        <div class="biz-donut-value" v-if="value" :style="{ fontSize: (size * 0.15) + 'px' }">{{ value }}</div>
        <div class="biz-donut-label" v-if="label" :style="{ fontSize: (size * 0.08) + 'px' }">{{ label }}</div>
      </slot>
    </div>
  </div>
</template>
...
export interface BizDonutSegment {
  value: number;
  color: string;
}

const props = withDefaults(defineProps<{
  data: BizDonutSegment[];
  size?: number;
  thickness?: number;
  trackColor?: string;
  label?: string;
