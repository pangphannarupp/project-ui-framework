<template>
  ...
  >
    ...
    <PPBottomSheet v-model="showMonthPicker" :title="pickerTitleText">
...
import { CalendarState, type CalendarDay, type CalendarConfig, type SelectionMode } from '../utils/KhmerDate';
...
const onDayClick = (day: CalendarDay) => {
  ...
  if (mode === 'Single') {
    ...
    emit('date-selected', { date: day.date });
  } else if (mode === 'Week') {
    ...
    if (index !== -1) {
      ...
      if (weekDays.length > 0) {
        ...
        emit('range-selected', 
          ...
          { date: weekDays[weekDays.length - 1].date }
        );
      }
    }
  } else {
    ...
    if (!rangeStartDate.value || (rangeStartDate.value && rangeEndDate.value)) {
      ...
      if (day.date.getTime() < rangeStartDate.value.getTime()) {
        ...
        emit('range-selected', 
          startDay ? { date: startDay.date } : null, 
          { date: day.date }
        );
      }
    }
...
const onConfirmAction = () => {
  ...
  if (props.config.selectionMode === 'Single') {
    emit('confirm', startDay ? { date: startDay.date } : null, null);
  } else {
    emit('confirm', 
      startDay ? { date: startDay.date } : null, 
      endDay ? { date: endDay.date } : null
    );
  }
};
</script>
...
.pp-calendar {
  ...
  --pp-calendar-subtitle-color: var(--pp-calendar-subtitle-text, rgba(26, 42, 94, 0.7));
  --pp-calendar-wheel-text: rgba(26, 42, 94, 0.4);
  --pp-calendar-wheel-active: var(--pp-calendar-text, var(--pp-primary-variant, #1a2a5e));
  --pp-calendar-sunday-color: var(--pp-danger-color, #ff4d4f);
  ...
  --pp-calendar-special-color: #FFA000;
  --pp-calendar-cell-height: 64px;
}
...
.btn-confirm-picker {
  ...
  border-radius: 8px;
  border: none;
  background: var(--pp-primary-color, var(--pp-primary, #003399));
  color: white;
  font-weight: 600;
  font-size: 16px;
  cursor: pointer;
...
.day-cell.is-selected .lunar-month {
  color: var(--pp-calendar-selected-text) !important;
}
...
.special-dot {
  ...
  height: 6px;
  border-radius: 50%;
  background: var(--pp-calendar-special-color);
}
...
.btn-confirm {
  background: var(--pp-danger-color, #ff4d4f);
  color: white;
  border: none;
  padding: 8px 16px;
  border-radius: 4px;
