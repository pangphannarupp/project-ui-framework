<template>
  <div class="biz-year-picker" :class="{ 'hide-actions': !showActionButtons }">
    <!-- Header Row -->
...
.biz-year-picker {
  ...
  width: 100%;
  background: var(--biz-calendar-bg, #fff);
  padding: 16px;
  border-radius: var(--biz-calendar-radius, 16px);
  user-select: none;
}
...
.year-title {
  font-size: 16px;
  font-weight: 600;
  color: var(--biz-calendar-text, #333);
}
...
.selection-overlay {
  ...
  height: 40px;
  margin-top: -20px;
  background: var(--biz-calendar-range-bg, rgba(255, 77, 79, 0.1));
  border-radius: 8px;
  pointer-events: none;
}
...
.wheel-item.is-selected {
  font-size: 22px;
  font-weight: 700;
  color: var(--biz-calendar-selected-bg, #ff4d4f);
}
