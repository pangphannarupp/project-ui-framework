<template>
  <div class="biz-time-picker" :class="{ 'hide-actions': !showActionButtons }">
    <!-- Header Row -->
...
.selection-overlay {
  ...
  height: 40px;
  margin-top: -20px;
  background: var(--biz-calendar-range-bg, rgba(0, 122, 255, 0.1));
  border-radius: 8px;
  pointer-events: none;
}
...
.wheel-item.is-selected {
  font-size: 22px;
  font-weight: 700;
  color: var(--biz-calendar-selected-bg, #007aff);
}
