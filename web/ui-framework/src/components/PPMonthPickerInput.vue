...
import PPMonthPicker from './PPMonthPicker.vue';

const props = withDefaults(defineProps<{
  modelValue?: { year: number, month: number } | null;
  placeholder?: string;
  clearable?: boolean;
  format?: (val: { year: number, month: number }) => string;
}>(), {
  modelValue: null,
...
const emit = defineEmits<{
  (e: 'update:modelValue', value: { year: number, month: number } | null): void;
}>();
...
const onMonthSelected = (payload: { year: number, month: number }) => {
